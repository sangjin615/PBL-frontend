<template>
  <div class="h-screen bg-gray-50 flex flex-col">
        ...
        <div class="flex items-center space-x-4">
          ...
          <div>
            <h1 class="text-lg font-semibold">
              {{ isEditMode ? "커리큘럼 수정" : "커리큘럼 만들기" }}
            </h1>
            <p class="text-sm text-gray-600">
              강의물을 조합하여 교육과정을 구성하세요
            </p>
          </div>
        </div>
        <div class="flex items-center space-x-4">
          <Button @click="previewCurriculum" variant="secondary">
            미리보기
          </Button>
          <Button
    ...
    <div class="flex flex-1 overflow-hidden">
      <!-- 에러/성공 메시지 -->
      <div
        class="absolute top-20 left-1/2 transform -translate-x-1/2 z-50 w-full max-w-md px-4"
      >
      ...
      <div class="w-1/3 bg-white border-r overflow-y-auto">
        <div class="p-6">
          ...
          <div class="mb-6">
            ...
            <!-- 학습 목표 -->
            <FormTextarea
              v-model="curriculum.learningObjectives"
              label="학습 목표"
              placeholder="이 커리큘럼을 통해 달성할 수 있는 학습 목표를 입력하세요. 각 목표는 줄바꿈으로 구분됩니다."
              :rows="5"
            />
          ...
          <div class="mb-6">
            ...
            >
              ...
              >
                <div class="flex items-center space-x-3">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <!-- 태그 표시 -->
                    <div
                      v-if="lesson.tags && lesson.tags.length > 0"
              ...
              <div
                ...
                :class="
                  isDraggingOverCurriculum ? 'border-blue-400 bg-blue-50' : ''
                "
              >
                <svg
            ...
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
                ...
                <span class="font-medium"
                  >{{ curriculum.lessons.length }}개</span
                >
              </div>
              <div class="flex justify-between">
      ...
      <div class="w-2/3 bg-gray-50 flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto p-6">
            ...
            >
              <div class="flex items-start justify-between">
                <div class="flex-1">
                  ...
                  <div
                    class="flex items-center space-x-4 text-xs text-gray-500 mb-2"
                  >
                  ...
                  <!-- 태그 표시 -->
                  <div
                    v-if="material.tags && material.tags.length > 0"
                    class="flex flex-wrap gap-1"
                  >
                    <span
          ...
          <div
            v-if="totalPages > 1"
            class="flex justify-center items-center py-4 border-t bg-white"
          >
            <div class="flex items-center space-x-4">
...
const titleError = computed(
  () => validateTitle(curriculum.value.title) || undefined
);
const descriptionError = computed(
  () => validateDescription(curriculum.value.description) || undefined
);
...
const totalPages = computed(() => {
  return activeTab.value === "all"
    ? allLecturesMeta.value.total_pages
    : myLecturesMeta.value.total_pages;
});

const totalCount = computed(() => {
  return activeTab.value === "all"
    ? allLecturesMeta.value.total_count
    : myLecturesMeta.value.total_count;
});
...
const filteredMaterials = computed(() => {
  // 탭에 따라 적절한 목록 선택
  let filtered =
    activeTab.value === "all" ? allLectures.value : myLectures.value;

  // 타입 필터
  if (filterType.value) {
    filtered = filtered.filter(
      (material) => material.type === filterType.value
    );
  }
...
const problemCount = computed(() => {
  return curriculum.value.lessons.filter((lesson) => lesson.type === "problem")
    .length;
});
...
const addTag = () => {
  if (
    tagInput.value.trim() &&
    !curriculum.value.tags.includes(tagInput.value.trim())
  ) {
...
async function uploadThumbnail(file: File) {
  ...
  try {
    ...
    curriculum.value.thumbnailImageUrl = response.imageUrl; // 경로만 저장 (예: "thumbnails/filename.png")
...
const onDropCurriculumItem = (event: DragEvent, targetIndex: number) => {
  ...
  if (draggedIndex.value !== null && draggedIndex.value !== targetIndex) {
    ...
    // 새 위치에 삽입
    const newIndex =
      draggedIndex.value < targetIndex ? targetIndex - 1 : targetIndex;
...
const loadMyLectures = async (page: number = 1) => {
  try {
    ...
    // 페이징된 응답 받기 (10개씩)
    const response = await lectureApiService.getUserLectures(
      currentUserId,
      backendPage,
      10
    );
...
const loadExistingCurriculum = async (curriculumId: number) => {
  ...
  try {
    ...
    curriculum.value.category =
      data.lectures?.[0]?.lectureCategory?.toLowerCase() || "";
    curriculum.value.difficulty = data.difficulty?.toLowerCase() || "";
    curriculum.value.summary = data.summary || "";
    curriculum.value.learningObjectives = data.learningObjectives || "";
  ...
  try {
    if (isEditMode.value && editCurriculumId.value) {
      ...
      await curriculumApiService.updateCurriculum(editCurriculumId.value, {
        ...
        learningObjectives: curriculum.value.learningObjectives,
      ...
      for (const lesson of curriculum.value.lessons) {
        await curriculumApiService.addLectureToCurriculum(
          curriculumResponse.id,
          {
            lectureId: lesson.id,
            isRequired: false,
          }
